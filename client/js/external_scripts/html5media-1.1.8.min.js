...
domReady();})();}
addLoadEvent(domReady);};DomReady.ready=function(fn,args){bindReady();if(isReady){fn.call(window,[]);}else{readyList.push(function(){return fn.call(window,[]);});}};bindReady();})();(function(window,document,undefined){"use strict";var VIDEO_TAG="video";var AUDIO_TAG="audio";if(!document.createElement(VIDEO_TAG).canPlayType){document.createElement(AUDIO_TAG);document.createElement("source");}
...
html5media.forceFallback=function(tagName,element){return false;};function dirname(path){return path.split("/").slice(0,-1).join("/")+"/";}
...
return"";}());html5media.flowplayerSwf=scriptRoot+"flowplayer.swf";html5media.flowplayerAudioSwf=scriptRoot+"flowplayer.audio.swf";html5media.flowplayerControlsSwf=scriptRoot+"flowplayer.controls.swf";html5media.expressInstallSwf=scriptRoot+"expressInstall.swf";var THEORA_FORMAT='video/ogg; codecs="theora, vorbis"';var H264_FORMAT='video/mp4; codecs="avc1.42E01E, mp4a.40.2"';var VORBIS_FORMAT='audio/ogg; codecs="vorbis"';var WEBM_FORMAT='video/webm;';var M4A_FORMAT='audio/x-m4a;';var MP3_FORMAT='audio/mpeg;';var WAV_FORMAT='audio/wav; codecs="1"';var assumedFormats={video:H264_FORMAT,audio:MP3_FORMAT};var fallbackFormats=[H264_FORMAT,M4A_FORMAT,MP3_FORMAT];var fileExtensions={video:{"ogg":THEORA_FORMAT,"ogv":THEORA_FORMAT,"avi":H264_FORMAT,"mp4":H264_FORMAT,"mkv":H264_FORMAT,"h264":H264_FORMAT,"264":H264_FORMAT,"avc":H264_FORMAT,"m4v":H264_FORMAT,"3gp":H264_FORMAT,"3gpp":H264_FORMAT,"3g2":H264_FORMAT,"mpg":H264_FORMAT,"mpeg":H264_FORMAT,"webm":WEBM_FORMAT},audio:{"ogg":VORBIS_FORMAT,"oga":VORBIS_FORMAT,"aac":M4A_FORMAT,"m4a":M4A_FORMAT,"mp3":MP3_FORMAT,"wav":WAV_FORMAT}};function guessFormat(tag,src,type){if(type){return type;}
var extensionMatch=(/\.([a-z1-9]+)(\?|#|\s|$)/i).exec(src);if(extensionMatch){var format=fileExtensions[tag][extensionMatch[1]];if(format){return format;}}
return assumedFormats[tag];}
function hasAttr(element,attr){var val=element.getAttribute(attr);return!!val||typeof val=="string";}
function fixPath(url){var link=document.createElement("a");link.href=url;return link.href;}
function getDimension(element,dimension,fallback){var result=element.getAttribute(dimension);if(result){return result+"px";}
var style;if(element.currentStyle){style=element.currentStyle[dimension];}else if(window.getComputedStyle){style=document.defaultView.getComputedStyle(element,null).getPropertyValue(dimension);}else{return fallback;}
...
html5media.configureFlowplayer=function(element,config){return config;};html5media.createFallback=function(tagName,element){var hasControls=hasAttr(element,"controls");var poster=element.getAttribute("poster")||"";var src=element.getAttribute("src")||"";if(!src){var sources=element.getElementsByTagName("source");for(var sn=0;sn<sources.length;sn++){var source=sources[sn];var srcValue=source.getAttribute("src");if(srcValue){for(var fn=0;fn<fallbackFormats.length;fn++){var fallbackFormat=fallbackFormats[fn];if(formatMatches(fallbackFormat,guessFormat(tagName,srcValue,source.getAttribute("type")))){src=srcValue;break;}}}
if(src){break;}}}
if(!src){return;}
...
var plugins={controls:hasControls&&{url:fixPath(html5media.flowplayerControlsSwf),opacity:0.8,backgroundColor:"#181818",backgroundGradient:"none",fullscreen:tagName==VIDEO_TAG,autoHide:tagName==VIDEO_TAG&&{fullscreenOnly:false,enabled:true,hideStyle:"fade",mouseOutDelay:0}||{enabled:false}}||null};if(isOpera&&plugins.controls){plugins.controls.autoHide.enabled=false;}
if(tagName=="audio"){plugins.audio={url:fixPath(html5media.flowplayerAudioSwf)};if(!hasControls){plugins.controls={url:fixPath(html5media.flowplayerControlsSwf),display:"none"};replacement.style.height=0;}
playlist[playlist.length- 1].autoBuffering=false;}
var config={play:null,playlist:playlist,clip:{scaling:"fit",fadeInSpeed:0,fadeOutSpeed:0},canvas:{backgroundGradient:"none",backgroundColor:"#000000"},plugins:plugins};config=html5media.configureFlowplayer(element,config);flowplayer(replacement,{src:fixPath(html5media.flowplayerSwf),expressInstall:fixPath(html5media.expressInstallSwf),wmode:"opaque"},config);};DomReady.ready(html5media);window.html5media=html5media;})(this,document);